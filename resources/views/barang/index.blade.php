            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    ...
                    <table class="w-full text-sm text-left text-gray-600">
                        ...
                        <tbody class="divide-y divide-gray-100" id="tableBody">
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors barang-row">
                                <td class="py-4 pr-4 text-center font-medium">{{ $barangs->firstItem() + $loop->index }}
                                </td>
                                <td class="py-4 px-4 whitespace-nowrap">{{ $item->created_at->format('d M Y') }}</td>
                                <td class="py-4 px-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-10 h-10 rounded-lg overflow-hidden flex-shrink-0 bg-blue-50 flex items-center justify-center border border-gray-100">
                                            @if($item->foto)
                                            <img src="{{ asset('storage/' . $item->foto) }}"
                                                alt="{{ $item->nama_barang }}" class="w-full h-full object-cover">
                                            @else
                                        ...
                                        <div>
                                            <div class="font-bold text-gray-900 barang-name">{{ $item->nama_barang }}
                                            </div>
                                            <div class="text-xs text-gray-500 barang-kode">{{ $item->kode_barang }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                ...
                <td class="py-4 px-4 text-center">
                    <div class="flex items-center justify-center gap-1.5">
                        ...
                        <a href="{{ route('barang.show', $item->kode_barang) }}" title="Detail Barang"
                            class="p-2 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-lg transition-colors shadow-sm">
                        ...
                        <a href="{{ route('barang.barcode', $item->kode_barang) }}" target="_blank"
                            title="Cetak Barcode"
                            class="p-2 bg-purple-50 hover:bg-purple-100 text-purple-600 rounded-lg transition-colors shadow-sm">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <a href="{{ route('barang.edit', $item->id) }}" title="Edit Barang"
                            class="p-2 bg-yellow-50 hover:bg-yellow-100 text-yellow-600 rounded-lg transition-colors shadow-sm">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <form action="{{ route('barang.destroy', $item->id) }}" method="POST" class="inline">
                            ...
                            <button type="submit" onclick="return confirm('Yakin ingin menghapus barang ini?');"
                                title="Hapus Barang"
                                class="p-2 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg transition-colors shadow-sm">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
