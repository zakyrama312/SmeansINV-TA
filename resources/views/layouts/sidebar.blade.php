    ...
    <div class="flex items-center justify-between h-16 px-4 bg-slate-950/50 border-b border-slate-800">
        <div class="flex items-center gap-3 overflow-hidden whitespace-nowrap">
            ...
            <span x-show="sidebarOpen" x-transition.duration.300ms
                class="font-bold text-lg tracking-wide text-white">Inventaris Lab</span>
        </div>
    </div>
        ...
        <div
            ...
            <div x-show="masterOpen && sidebarOpen" x-transition.opacity class="pl-11 pr-3 py-2 space-y-1">
                <a href="{{ route('prodi.index') }}"
                    class="block px-3 py-2 rounded-lg text-sm transition-colors {{ request()->routeIs('prodi.*') ? 'bg-blue-600/20 text-blue-400 font-semibold' : 'text-slate-400 hover:text-white hover:bg-slate-800' }}">
                    Prodi
                </a>
                <a href="{{ route('ruang.index') }}"
                    class="block px-3 py-2 rounded-lg text-sm transition-colors {{ request()->routeIs('ruang.*') ? 'bg-blue-600/20 text-blue-400 font-semibold' : 'text-slate-400 hover:text-white hover:bg-slate-800' }}">
                    Ruang Lab
                </a>
                <a href="{{ route('kategori.index') }}"
                    class="block px-3 py-2 rounded-lg text-sm transition-colors {{ request()->routeIs('kategori.*') ? 'bg-blue-600/20 text-blue-400 font-semibold' : 'text-slate-400 hover:text-white hover:bg-slate-800' }}">
                    Kategori
                </a>
                <a href="{{ route('kondisi.index') }}"
                    class="block px-3 py-2 rounded-lg text-sm transition-colors {{ request()->routeIs('kondisi.*') ? 'bg-blue-600/20 text-blue-400 font-semibold' : 'text-slate-400 hover:text-white hover:bg-slate-800' }}">
                    Kondisi
                </a>
            </div>
        </div>
